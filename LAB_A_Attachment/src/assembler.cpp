...
std::string assembler::TranslateOprand(unsigned int current_address, std::string str, int opcode_length){
    ...
    if (item != -1) {
        // str is a label
        // TO BE DONE
        auto temp = item - current_address - 1; 
...
std::string assembler::LineLabelSplit(const std::string &line,
    ...
    if (IsLC3Pseudo(first_token) == -1 && IsLC3Command(first_token) == -1 &&
        ...
        // TO BE DONE
        //TODO:
        label_map.AddLabel(first_token, current_address);        //address?
...
std::string assembler::TranslatePseudo(std::stringstream &command_stream) {
    ...
    if (pseudo_opcode == ".FILL") {
        ...
        std::string number_str;
        command_stream >> number_str;
        auto cnt = RecognizeNumberValue(number_str);
        ...
        for (int i = 1; i < size - 1; i++) {
            int temp = str[i];
            auto bin = NumberToAssemble(temp);
            if (gIsHexMode) bin = ConvertBin2Hex(bin);
            out << bin << std::endl;
        }

        if (gIsHexMode) out << "0000"; 
        else  out << "0000000000000000";
        output_line = out.str();
